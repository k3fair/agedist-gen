...
for i in range(df.shape[0]):
    ...
    algthresh = 1e-4 # Set the threshold for stoppping the fitting algorithm
    maxsteps = 250 # Maximum number of steps to run of the fitting algorithm
    ...
    while step < maxsteps:
        ...
        if fitness[best_idx] < best_fitness:
            ...
            plt.tight_layout()
            # plt.savefig(f'{home}data/gifimages/agedist_activationmodel_{df.iloc[numselect,3]}_fig{figcapture}_model2_thresh{algthresh}_maxsteps{maxsteps}.png', bbox_inches="tight", dpi=300)
            plt.savefig(f'{home}data/agedist_{df.iloc[numselect,3]}_model2_thresh{algthresh}_maxsteps{maxsteps}.png', bbox_inches="tight", dpi=300)
            plt.show()
        ...
        if best_fitness < algthresh:
            ...
            pd.DataFrame(best_sol).to_csv(f'{home}data/bestparms_{df.iloc[numselect,3]}_model2_thresh{algthresh}_maxsteps{maxsteps}.csv') 
            print("Desired accuracy has been achieved, ending fitting.") 
            successful_attempts.append(df.iloc[numselect,3])
    ...
    print(f'Completion percentage: {(i+1)/df.shape[0]}')


print(f'We successfully found paramterisations for {success_count} of the {df.shape[0]} age distributions using model 2.')

#Store lists of which age distributions we were(n't) able to calibrate for successfully
df_successful = pd.DataFrame(successful_attempts)
df_failed = pd.DataFrame(failed_attempts)

df_successful.to_csv(f'{home}data/agedists_other_model2successful_thresh{algthresh}_maxsteps{maxsteps}.csv', index=False,encoding='utf-8-sig') 
df_failed.to_csv(f'{home}data/agedists_other_model2failed_thresh{algthresh}_maxsteps{maxsteps}.csv', index=False,encoding='utf-8-sig') 
